...
export const useScrollFunctionality = ({
  ...
  // Apply the first chapter's onChapterEnter opacities as soon as the map
  // loads, before any Waypoint has fired. Without this, layers that should
  // start hidden are briefly visible at their default (usually 1) opacity
  // until the user scrolls and the first chapter's enter trigger fires.
  useEffect(() => {
...
export const useHandleResize = (callback) => {
  ...
  // Keep a ref to the latest callback so the listener (set up once) always
  // calls current logic instead of closing over stale values (e.g. a `map`
  // instance that was still null when the effect first ran).
  ...
  useEffect(() => {
    const getSize = () => {
      if (size[0] !== window.innerWidth || size[1] !== window.innerHeight) {
        callbackRef.current({ width: window.innerWidth, height: window.innerHeight });
